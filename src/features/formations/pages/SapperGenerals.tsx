// src/features/formations/pages/SapperGenerals.tsx
...
export default function SapperGenerals() {
  ...
  const { create } = useSapperGeneralsMutations();
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="mb-4">
        <h1 className="text-xl font-semibold">Formations – Sapper Generals</h1>
        <div className="mt-2">
          <FormationsTabs active="generals" />
        </div>
      </header>
      ...
      <div className="rounded-xl border p-4 space-y-3">
        <div className="grid md:grid-cols-4 gap-3">
          ...
          <input
            ...
            type="number"
            value={f.position}
            onChange={(e) => setF((s) => ({ ...s, position: Number(e.target.value) }))}
          />
          <input
            className="border rounded-lg px-3 py-2 text-sm"
            placeholder="Content"
          ...
          <input
            className="border rounded-lg px-3 py-2 text-sm"
            type="file"
            onChange={(e) => setF((s) => ({ ...s, image: e.target.files?.[0] ?? null }))}
          />
      ...
      <div className="rounded-xl border overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <Row key={String(r.id)} item={r} refetch={refetch} />
            ))}
          </tbody>
        </table>
      </div>
  ...
  return (
    <tr className="[&>td]:px-3 [&>td]:py-2 border-t">
        ...
        <label className="inline-flex items-center gap-2">
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              if (!file) return;
              await updateImage.mutateAsync({ general_id: item.id, image: file });
              refetch();
            }}
          />
        </label>
      ...
      <td className="space-x-2">
        {edit ? (
          <>
            <button onClick={save} className="rounded-lg border px-2 py-1 hover:bg-zinc-50">
              Save
            </button>
            <button onClick={() => setEdit(false)} className="rounded-lg border px-2 py-1 hover:bg-zinc-50">
              Cancel
            </button>
          </>
        ) : (
          <>
            <button onClick={() => setEdit(true)} className="rounded-lg border px-2 py-1 hover:bg-zinc-50">
              Edit
            </button>
            <button
              onClick={async () => {
                if (!confirm("Delete general?")) return;

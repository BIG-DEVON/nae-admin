// src/features/formations/pages/SapperChronicles.tsx
...
export default function SapperChronicles() {
  ...
  // ✅ Fix: valid union type (no pipeline operator)
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="mb-4">
        <h1 className="text-xl font-semibold">Formations – Sapper Chronicles</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-zinc-50/60">
            <tr className="[&>th]:px-3 [&>th]:py-2 text-left">
              <th>ID</th><th>Title</th><th>Sub</th><th>Pos</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
  ...
  return (
    <tr className="[&>td]:px-3 [&>td]:py-2 border-t">
      ...
      <td className="space-x-2">
        <button
          onClick={()=>onSave({ title, sub_title: sub || undefined, position: Number(pos || 0) })}
          className="rounded-lg border px-2 py-1 hover:bg-zinc-50"
        >
          Save
        </button>
        <Link
          to={`/formations/sapper-chronicles/contents?section_id=${item.id}`}
          className="rounded-lg border px-2 py-1 hover:bg-zinc-50"
        >
          Contents
        </Link>

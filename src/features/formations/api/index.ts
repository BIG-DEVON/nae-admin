...
async function sendForm<T>(path: string, fd: FormData): Promise<T> {
  const r = await fetch(`${BASE}${path}`, {
    method: "POST",
    headers: { ...authHeader() }, // let browser set boundary
    body: fd,
  });
  const text = await r.text();
  if (!r.ok) throw new Error(text || r.statusText);
...
/* ---------- Public GETs ---------- */
export const getChronicles = async () =>
  toArray(await getJSON<unknown>("/formations/chronicles/"));
 ...
 * If backend expects chronicles_id instead, change the param name here.
 */
...
export const createChronicles = (payload: {
  title: string;
  position: number;
}) => sendJSON("/formations-actions/chronicles/", "POST", payload);

export const updateChronicles = (payload: {
  chronicles_id: ID;
  title?: string;
  position?: number;
...
export const createChronicleSection = (payload: {
  chronicles_id: ID;
  title: string;
  position: number;
}) =>
...
export const deleteChronicleSection = (section_id: ID) =>
  sendJSON("/formations-actions/chronicles/sections/", "DELETE", {
    section_id,
  });
...
export const deleteChronicleContent = (content_id: ID) =>
  sendJSON("/formations-actions/chronicles/contents/", "DELETE", {
    content_id,
  });
...
export const updateSapperGeneralImage = (payload: {
  general_id: ID;
  image: File;
}) => {
  const fd = new FormData();
...
export const createSapperChronicles = (payload: {
  title: string;
  position: number;
  sub_title?: string;
}) =>
...
export const deleteSapperChronicles = (chronicles_id: ID) =>
  sendJSON("/formations-actions/sapper-chronicles/", "DELETE", {
    chronicles_id,
  });
...
export const deleteSapperChroniclesContent = (content_id: ID) =>
  sendJSON("/formations-actions/sapper-chronicles/contents/", "DELETE", {
    content_id,
  });

/* ---------- Compatibility aliases (keep old imports working) ---------- */
export const fetchChronicleSections = getChronicleSections;
export const fetchChronicleContents = getChronicleContents;
export const createChronicle = createChronicles;
export const updateChronicle = updateChronicles;
export const deleteChronicle = deleteChronicles;
export const removeChronicleContent = deleteChronicleContent;
